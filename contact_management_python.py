...
###########################################################################################################
      ...
      def user_defined_gui_window_method(self):
            ...
            ##################################################################################################
            #
            # Set these GLOBALS to control selection of MEDICAL RECORD GROUP (ONE, TWO, THREE)
            #
            # These are set using this command (for example): 
            #
            # str(instance_object_focus.winfo_parent() ) = .!toplevel9
            #
            # global gui_group_one_object
            # global gui_group_two_object
            # global gui_group_three_object 
...
class USER_GUI_Config_Class(Frame):    #(object)
    def __init__(self, master):
        ...
        if user_defined_gui_instance_count_GLOBAL == 3:
        ...
        # ERROR - CORRECT THIS -  if "toplevel3" because TOPLEVELS, appearing in self.master,
        # can be any number, after instances are destroyed and re-created ..... 
        # 
        # Also, try setting these in App Class immediately after instantiating this USER DEFINED CLASS:
        # gui_group_three_object, gui_group_two_object, and gui_group_one_object 
        #
        # and search for all other code containing:  if "toplevel3" in str(self.master):

        if self.USER_GUI_Config_Class_INSTANCE_ID == "INSTANCE_THREE":
    ...
    def very_first_create_medical_record_config_ini(self):
        ...
        if os.path.isfile(fullpath_med_config_ini_global) == False:  # self.very_first_create_medical_record_config_ini()
            ...
            very_first_medical_config.set("USER_DESIGNS_GUI", "group3_frame8_user_label", "Potassium - mmol/L")
            very_first_medical_config.set("USER_DESIGNS_GUI", "group3_frame8_status_text", "Enter Potassium Level below ...")
            very_first_medical_config.set("USER_DESIGNS_GUI", "group3_frame8_user_button", "Record Potassium")
