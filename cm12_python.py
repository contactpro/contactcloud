...
###########################################################################################################
      ...
      def crm_startup_screen_show_method(self):  
          ...
          self.after(1000, lambda: self.new_list_window_method() )

          self.after(2000, lambda: self.new_window_method() )

          self.after(3000, lambda: self.view_mode_method() )

          self.after(4000, lambda: self.export_CSV_for_Excel_method() )

          self.after(5000, lambda: self.build_list_from_dual_listbox_window_method() )

          self.after(6000, lambda: self.cm_app_doc_media_window_method() )

          self.after(7000, lambda: self.email_Gmail_Feature_method() )

          self.after(8000, lambda: self.select_App_Status_Display_method() )

          self.after(9000, lambda: self.config_App_Settings_method() )

          self.after(10000, lambda: OBJECT_main.lift() )

          self.after(10100, lambda: self.COMMAND_CENTER_STARTUP_method() )
      ...
      def COMMAND_CENTER_STARTUP_method(self):   
          ...
          self.after(100, lambda: self.COMMAND_CENTER_STARTUP_SCREEN_black_blue() )

          self.after(8000, lambda: self.COMMAND_CENTER_STARTUP_SCREEN() )
      ...
      def night_mode_select_method(self): 
          ...
          if night_mode_selection == False:
              night_mode_selection = True
              self.COMMAND_CENTER_STARTUP_SCREEN_black_blue()
          elif night_mode_selection == True:
      ...
      def COMMAND_CENTER_STARTUP_SCREEN(self): 
          self.entry_first.set("      CONTACT MANAGEMENT")
          self.myentry1.config(background="dark slate gray", fg="cyan3")
          self.entry_last.set("      CUSTOMER RELATIONSHIP")
          self.myentry2.config(background="dark slate gray", fg="cyan3")
          self.entry_streetadd.set("                and ")
          self.myentry3.config(background="dark slate gray", fg="cyan3")
          self.entry_citytown.set("      PERSONAL HEALTH")
          self.myentry4.config(background="dark slate gray", fg="cyan3")
          self.entry_state.set("      MEDICAL PHYSIOLOGY")
          self.myentry5.config(background="dark slate gray", fg="cyan3")
          self.entry_zipcode.set("      COMMAND CENTER")
          self.myentry6.config(background="dark slate gray", fg="cyan3")
          self.entry_phonenum.set("*** Version 12.0 ")
          self.myentry7.config(background="dark slate gray", fg="cyan3")
          self.entry_email.set("*** December 25, 2017 ") 
          self.myentry8.config(background="dark slate gray", fg="cyan3")
          self.entry_website.set("*** Python 3.6.2 - Windows 10 ") 
          self.myentry9.config(background="dark slate gray", fg="cyan3")
          self.master.update()
