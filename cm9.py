            ...
            for c in scroll_label:
                  if r > 2 and r < 9:
                       if r == 3:
                             ...
                             width=12,height=2, font=medium_font, \
                             background="cyan4", fg = "cyan", command = self.backward_scroll)
...
class Demo2_List_Builder(object):
    def __init__(self, master):
        ...
        self.GET_LBOX_DESTINATION_curselection_Button = Button(self.master, text = "REMOVE from LIST\n(Shift SELECTS Group)", \
        width = 20, height = 2, font=('Helvetica', '16'), \
        background="cyan4", fg="black", activebackground="cyan", activeforeground="blue2", \
        ...
        self.GET_LBOX_SOURCE_curselection_Button = Button(self.master, text = "ADD to LIST\n(Shift SELECTS Group)", \
        width = 20, height = 2, font=('Helvetica', '16'), \
        background="cyan4", fg="black", activebackground="cyan", activeforeground="blue2", \
        ...
        self.source_file_status_text_box.config(borderwidth=10, font=('Helvetica', '12'), background="cyan")

        source_file_status_TEXT = "Select CONTACT LIST below ...."
        ...
        self.lbox = Listbox(self.master, width=42, height = 22)
        ...
        for root,dirs,files in os.walk(testdir):
            for f in files:
                 if ( (f.endswith('.txt') and ("cm_list_" in str(f) ) ) ):
                     ...
                     try:
                           split_cm_list_see_bracket_ONE = f.split("cm_list_")[1]
                           ...
                           print(".... IndexError EXCEPTION when splitting cm_list_ and dot_txt to populate LISTBOX : We Will pass")
                           pass
...
#123456789012
    ...
    def get_Listbox_File(self, event):
        ...
        for each_contact in contact_list_for_listbox:
              self.lbox_Source_Contacts.insert(0, each_contact)


        #  if self.widget==str(".!toplevel.!listbox2"):  
    ...
    def OnListBoxSelect(self, event):
        ...
        print(".... See what this is:   selection[0]  =  " + str(selection[0] ) )
        ...
        # WE ARE COMMENTING THIS OUT NOW BECAUSE WE DO ALL THE
        # LISTBOX SELECT ACTIVITY IN METHODS AT THE TOP NOW ....
        # THAT UTLIZE SELECTMODE = EXTENDED ......
        
        ###   self.LISTBOX_SEL_LIST.append(str(selection[0] ) )
...
#.... widget = event.widget = .!toplevel.!listbox 
#.... widget = event.widget = .!toplevel.!listbox2  
#.... widget = event.widget = .!toplevel.!listbox3 
#
#.... EVENT  <Button-1>  = <VirtualEvent event x=0 y=0>
#.... widget = event.widget = .!toplevel.!listbox2
#.... selection = widget.curselection() = (14,)
#.... contact_string_value = widget.get(selection[0]) = Elliot2 FFFF2 theirmail@gmail.com2
#
#.... EVENT  <Button-1>  = <VirtualEvent event x=0 y=0>
#.... widget = event.widget = .!toplevel.!listbox2
#.... selection = widget.curselection() = (15,)
#.... contact_string_value = widget.get(selection[0]) = Tim9 EEEE9 othermail@gmail.com9
...
#12345678
        ...
        print("\n") 
        print(".... N E W   D I C T I O N A R Y   B U I L D  -  SELECTED and LOADED - fullpath_fn_dict_filename_global =  \n" + str(fullpath_fn_dict_filename_global) )
        print("\n") 
    ...
    def create_the_NEW_DICTIONARY(self):
        ...
        ################################################################################################
        # 
        # NOTE: We are also demonstrating using the get command to acquire the tuple of all
        # items in the Destination Listbox (see built_lbox_items line of code below).
        #
        # Note, however, that although the GET acquires the Destination Listbox items,
        # we were not interested in getting the listbox3 items because
        # we specifically need the (listbox2) Middle Listbox selection indexes to put
        # them in a list and map them back to the ORIGINAL DICTIONARY of DICTIONARIES Object
        # with the formula: 
        #
        #      self.DICT_INDEX  = self.TOTAL_RECORDS - int(listbox_index)
        #
        # so that we can utilize the listbox selections of items that were selected in listbox2
        # mapped to the original dictionary records list and then read ALL THE DATA in the
        # original dictionary corresponding to the records associated with our listbox2
        # selections. This line of code, however, could be helpful in the future, so we
        # note it here:
        #
        #      built_lbox_items = self.lbox_Destination_Contacts.get(0, END)
        #
        ...
        print("ALSO MAY USE the GET to get all the FINAL LISTBOX ITEMS.\nself.lbox_Destination_Contacts.get(0, END) = \n" + str(built_lbox_items) )
        ...
        #
        #  and then compute the  self.DICT_INDEX via the mapping
        #
        #  self.DICT_INDEX  = self.TOTAL_RECORDS - self.LISTBOX_SEL
        #
        ...
        ###############################################################################################
        #
        # NOTE: DICT_INDEX  = TOTAL_RECORDS - LISTBOX_SEL
        #
        ###############################################################################################
        #
        # self.TOTAL_RECORDS ............ self.TOTAL_RECORDS
        # self.LISTBOX_SEL_LIST ......... self.LISTBOX_SEL_LIST
        # self.DICT_INDEX ............... self.DICT_INDEX  = self.TOTAL_RECORDS - self.LISTBOX_SEL_LIST
        #
        # self.DICT_INDEX  = self.TOTAL_RECORDS - self.LISTBOX_SEL
        #
        # WE BUILD NEW DISCTIONARY OF DICTIONARIES by Indexing:
        #
        # self.SELECTED_DICT_OF_DICT_GLOBAL with our LIST of SELECTION:
        #
        # self.LISTBOX_SEL_LIST converted with this formula:
        #
        # self.DICT_INDEX  = self.TOTAL_RECORDS - self.LISTBOX_SEL_LIST
        #
        ###############################################################################################
        ...
        for listbox_index in self.LISTBOX_SEL_LIST:
             ...
             print("  ")
             print(".... COMPUTING DICT INDEX RE-MAP:   self.DICT_INDEX  = self.TOTAL_RECORDS - int(listbox_index)")
             print("  ")
             print(".... self.TOTAL_RECORDS = " + str(self.TOTAL_RECORDS) )
...
#12345678901234
              for count in range(0,4):
                  ...
                  self.select_file_button.config(background = "goldenrod1")
                  self.source_file_status_text_box.config(background = "goldenrod1")
                  ...
                  self.my_cm_filename_entry.config(background = "goldenrod1")
                  self.completed_new_contact_list_name_button.config(background = "goldenrod1")
                  ...
                  self.create_the_new_dictionary_button.config(background = "goldenrod1")
                  self.final_build_status_text_box.config(background = "goldenrod1")
...
#12345678901234
              self.select_file_button.config(background = "red2")
              self.source_file_status_text_box.config(background = "red2")
        
              self.my_cm_filename_entry.config(background = "red2")
              self.completed_new_contact_list_name_button.config(background = "red2")
        
              self.create_the_new_dictionary_button.config(background = "red2")
              self.final_build_status_text_box.config(background = "red2")
...
#12345678901234
              self.select_file_button.config(background = "blue")
              self.source_file_status_text_box.config(background = "blue")
        
              self.my_cm_filename_entry.config(background = "blue")
              self.completed_new_contact_list_name_button.config(background = "blue")
        
              self.create_the_new_dictionary_button.config(background = "blue")
              self.final_build_status_text_box.config(background = "blue")
