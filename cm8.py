...
###########################################################################################################
      ...
      #
      #   mode_select_global == "Entry Mode"   or   "Edit Mode"  
      #
      #####################################################################################
      def decision_SAVE_CONTACT_ENTRY(self):
          global mode_select_global
            
          if mode_select_global == "Entry Mode":
              self.finished_Data_Entry()
          elif mode_select_global == "Edit Mode":
      ...
      def finished_Data_Entry(self):
            ...
            print(".... mode_select_global = " + str(mode_select_global) )
      ...
      def __init__(self, master, **kw):
            ...
            *List_of_Textbox_Edit_Modes, command=self.func_set_textbox_edit_mode_select_global)
            ...
            self.lower_window_Button = Button(self.master, text = "CYCLE", width = 7, height = 1, \
                font=('Helvetica', '16'), background="cyan4", borderwidth=5, command = self.lower_email_WINDOW)

            self.lower_window_Button.grid(row=7, column=0)

            #
            # EXIT BUTTON.  
            # 
            self.quitButton = Button(self.master, text = "EXIT", width = 7, height = 1, \
                font=('Helvetica', '16'), background="cyan4", borderwidth=5, command = self.exit_Handler)

            self.quitButton.grid(row=7, column=0, sticky = NW)
...
class App_Status_Class(Frame):    #( object)
    def __init__(self, master=None):
        ...
        #
        self.quit_status_panel_Button = Button(self.master, text = "Lower Window on Stack\n(Better for Thread)", \
        width = 26, height = 2, font=('Helvetica', '12'), \
        background="ivory4", fg="black", command = self.lower_the_window)
...
class CM_App_Doc_Media(object):
    def __init__(self, master):
        ...
        # QUIT Button - Return to MAIN SCREEN.
        self.quitButton = Button(self.master, text = 'CLICK to Return\nto Main Screen', width = 34, height = 4, \
            font=('Helvetica', '12'), background="cyan4", command = self.close_windows)
...
class Demo5(object):
    def __init__(self, master):
        ...
        # Button Widget for ROW ZERO to Reserve ROW ZERO for Expanded TOOLBAR Type Button Functions.
        self.reserve_row_zero_button = Button(self.master, text = "", \
            width=25,height=3, font=('Helvetica', '12'), background="dark slate gray")
        ...
        # QUIT Button - Return to MAIN SCREEN.
        self.quitButton = Button(self.master, text = 'CLICK to Return\nto Main Screen', width = 25, height = 4, \
            font=('Helvetica', '12'), background="cyan4", command = self.close_windows)
